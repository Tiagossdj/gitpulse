...
export const githubRepository: GitHubRepository = {
  async getRepoHealth(owner, name) {
    ...
    const fetchGH = async (endpoint: string) => {
      const res = await fetch(`https://api.github.com${endpoint}`, { headers : headers as Record<string, string> });
      if (!res.ok) {
        if (res.status === 404) throw new AppError("Repositório não encontrado", 404);
    ...
    const [repo, pulls, issues, contributors] = await Promise.all([
      ...
      fetchGH(`/repos/${owner}/${name}/pulls?state=open&sort=created&per_page=5`),
    ...
    return {
      ...
      staleIssues: staleIssues.map((issue: any) => ({
        id: issue.id,
        title: issue.title,
        url: issue.html_url,
